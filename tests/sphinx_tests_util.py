...
"""
    ...
    This is a slightly modified version of Sphinx' built-in
    tests.util module. It's duplicated here because the
    Sphinx' built-in module isn't packaged along with Sphinx, and the
    sphinx-testing package (as of 2016-05-29) isn't compatible with
    the current (v1.4.1) Sphinx due to insufficient cleanup of
    nodes, roles, etc. registered during testing.
"""
...
tempdir = path(tempfile.mkdtemp(prefix="sphinxcontrib-traceables"))
...
class TestApp(application.Sphinx):
    ...
    def __init__(self, buildername='html', testroot=None, srcdir=None,
                 ...
                 tags=None, docutilsconf=None, warningiserror=False):
        if testroot is None:
            defaultsrcdir = 'root'
